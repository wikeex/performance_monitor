...
async def start_monitor():
    ...
    return web.json_response({
        ...
        'data': {
            'ip': master.slaves['ip'],
            'foos': monitor_list,
            'run_status': ['已停止', '监控中', '排队中']
        },
        'msg': None
    })


@routes.post('/visualize')
...
async def plot_monitor(request):
    ...
    if host in master.slaves['ip']:
        try:
            ...
            if type_ == 'port' or type_ == 'pid':  # 如果选择端口和pid，则可视化端口的CPU、内存，统计系统的IO和带宽
...
async def main():
    ...
    app.router.add_static('/static/',
                          path=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'static'))  # 将静态文件添加到搜索路径
